...
class FasterWhisperASR(ASRBase):
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        # or run on GPU with INT8 (works on my GTX1050)
        ...
        # model = WhisperModel(model_size_or_path, device="cpu", compute_type="int8", download_root=cache_dir) #, download_root="faster-disk-cache-dir/")
        return model
...
def asr_faster_whisper(logfile=sys.stderr):
	"""
	...
	print(f"Loading Whisper {size} model for en...", file=logfile, end=" ", flush=True)
	asr = asr_cls(modelsize=size, lan="en", cache_dir=cache_dir, model_dir=None)
	e = time.time()
