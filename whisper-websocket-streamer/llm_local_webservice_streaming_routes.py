...
def add_local_llm_streaming_routes(app):

	
	# LLM
	#model_name = "pastiche-crown-clown-7b-dare.Q4_0.gguf"
	print("LOAD LOCAL LLM",LLM_MODEL,LLM_MODEL_PATH)
	...
	@app.websocket("/llm/local/{client_id}/ws")
...
# ### User:			"""
			system_prompt = """
			You are a home assistant that listens for commands to turn devices on and off.
For example
Turn on the loungeroom light.
You should respond in JSON format similar to below.
[{'action':'turn_on', target:'loungeroom light' }]
"""
			kill_me = False
				...
				while True:
					while not is_stopped[client_id]:
						...
						if len(shared_list) > 0:
							...
							# finished
							n = time.time() - t
							# skip finished message if explicitly stopped
							if not is_stopped[client_id]:
